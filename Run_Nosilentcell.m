function [DataOut]=Run_Nosilentcell(data, samefile, cafirst,redobackground, threshhold)
%% LOADING IMAGES
DataOut = data;
...
<<<<<<< Updated upstream
st=1; %changed to 30 -- typically start at 1
=======
st=150;
>>>>>>> Stashed changes
ed=size(Images,3);
...
if isempty(mask)
    figure
    imshow(mat2gray(MaxIntensityMap)); %imagesc - another function for showing images in color
    numAreas=1;
    mask=zeros(sx,sy);
    disp('select islet')
    for i=1:numAreas
        bf=imfreehand(); %allows you to choose individual regions
...
% ActMap=zeros(sx,sy);
% %Determine the active area using the background threshhold
% ActMap(peakAmpMap>=threshold*scthresh)=1;
% ActMapNoGFP=ActMap;
% 
% ActMap(~LogicalMap)=0;
...
ActMap=zeros(sx,sy);
%Determine the active area using the background threshhold
ActMap(peakAmpMap>=2*scthresh)=1;
ActMapNoGFP=ActMap;
...
%%%%%%Use the CoorMap to overlay the activity map to put a number to activity removing regions in the no signal mask, outside of the activity mask, and the image mask
...
if (max(max(ActMask))>0)
    ...
    CorrPeakAmp = Dat.Img();
    figure; imagesc(CorrPeakAmp)

%     CorrPeakAmp = CorrPeakAmp.*ActMask;
%     figure; imagesc(CorrPeakAmp);
%     
%     diffmap = logical(ActMask) - logical(CorrPeakAmp);
%     connected=bwlabeln(diffmap,8);
%     connected = connected + max(max(CorrPeakAmp));
%     connected = connected.*ActMask - max(max(CorrPeakAmp))*logical(CorrPeakAmp);
%     CorrPeakAmp = CorrPeakAmp+connected;
%     figure; imagesc(CorrPeakAmp);
...
imagesc(CorrPeakAmp);
colorbar;
title('Coorelation Map')

disp('End of analysis.')
%disp('Active Area is %.4f %%', percentActiveArea)
end